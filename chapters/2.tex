...
W celu przygotowania listy wyra¿eñ u¿ytych do przeszukiwania zbiorów podjêto nastêpuj¹ce kroki:
\begin{enumerate}
	\item Przet³umaczenie pytañ na jêzyk angielski.
		\begin{itemize}
		...
		\end{itemize}
	\item Wypisanie s³ów kluczowych dotycz¹cych pytañ badawczych.
		\begin{itemize}
			\item defect prediction, method level, efficiency, extract, source changes, code changes
		\end{itemize}

	\item Wypisanie synonimów i alternatywnych form.
		\begin{itemize}
	...
	\item Utworzenie form z gwiazdk¹ (*) zastêpuj¹cych liczbê pojedynczê i mnog¹ oraz alternatywne formy i koñcówki.
		\begin{itemize}
	...
	\item Wykonanie próbnego przeszukiwania a nastêpnie wybranie s³ów kluczowych ze znalezionych trafnych wyników (brano pod uwagê s³owa kluczowe oraz streszczenie).

	\item Przygotowanie z³o¿onych zapytañ wykorzystuj¹c dostêpne operatory:
		\begin{itemize}
			\item {\bf IEEE Xplore}:
				\begin{itemize}
...
Wstêpne przeszukiwanie wykaza³o niewielk¹ liczbê Ÿróde³ œciœle odpowiadaj¹cych zagadnieniu predykcji defektów na niskim poziomie granulacji. Aby pokryæ znaczn¹ czêœæ szarej literatury zdecydowano, aby przeszukaæ nastêpuj¹ce Ÿród³a:
\begin{itemize}
	\item Google Scholar.
		\\U¿yto nastêpuj¹cych wyra¿eñ:
		\begin{itemize}
			\item (method AND level) AND (defect* AND predict*)
			\item (fine AND grain*) AND (defect* AND predict*)
		\end{itemize}
		Dla ka¿dego z wyra¿eñ przejrzano pierwsze 100 wyników.
...
\begin{itemize}
	...
	\end{itemize}
	\item Metryki procesu (inaczej metryki zmian) \cite{madeyski2014process}. Okreœlaj¹ zmiennoœæ atrybutu w~czasie. Oblicza siê je dla zadanych przedzia³ów czasowych. Niezbêdna do ich obliczenia jest historia projektu, któr¹ mo¿na uzyskaæ dziêki systemom kontroli wersji (jak SVN czy Git). Przyk³ady metryk procesu:
		\begin{itemize}
